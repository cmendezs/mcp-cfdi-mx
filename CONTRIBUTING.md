...
```bash
uv run ruff check src/mcp_cfdi_mx/ tests/ audit/
uv run ruff format --check src/mcp_cfdi_mx/ tests/ audit/
uv run mypy src
```
...
```bash
uv run ruff check --fix src/mcp_cfdi_mx/ tests/ audit/
uv run ruff format src/mcp_cfdi_mx/ tests/ audit/
```
...
- [ ] Validation fixes reference the relevant rule ID (e.g. `CFDI40-Anexo20-III.A`)
...
Please open an issue at https://github.com/cmendezs/mcp-cfdi-mx/issues and include:
